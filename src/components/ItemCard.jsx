  ...
  return (
    <View className="mx-2 py-2 px-3 border-2 border-gray-700 rounded-lg justify-between items-start mb-2">
      <View className="w-full flex-row justify-between items-center mb-1">
        <Text className="text-lg font-semibold text-headline">
          {item.title[0].toUpperCase()}
          {item.title.slice(1)}
        </Text>
        <Text className="text-headline text-sm ">
          ${item.price}/{item.divider.length} ={' '}
          <Text className="text-headline text-lg font-bold">${divided_price}</Text>
        </Text>
      </View>
      <FlatList
        horizontal
